let shouldAutoPlayMidi = false;

/* Midi stuff */
function initMidi(){
	// Removes hover effect if votouchMidi is enabled
	const votouchMidi = 'ontouchstart' in document.documentElement
	...
	*/
	...
	$('.midi-track-button').click(function(){
		...
		const thisTarget = $(this).attr('track-id');
	...
	$('#seek-slider-midi').rangeslider({
		...
		onSlide:function(position, value){
			if(isNaN(midiAudio.duration)){
				...
				$('#midi-current-time').text(secondToString(midiAudio.duration * (value/100)));
			}
...
/*
	...
	======================================================================
*/
const midiExtension = ".mp3";

/* File name of the tracks */
const midiPlaylist = ["01", "02", "03", "04", "05", "06", "07", "08", "09", "10", "11"];

/* Track title */
const midiTrackTitle = [
"Welcome Back", "Night Season", "Air Forest", "Bar Fight", 
"Punk", "Sherd Master", "Gear Up", "RAWR", "Panic", "Insomnia", "See Ya"];

/* Track compose date */
const midiTrackDate = ["September 4, 2015", "March 5th, 2017", "April 15, 2010", "December 4, 2012", "May 19, 2015",
"Unknown Date", "Febuary 29, 2016", "September 27, 2014", "January 27, 2012", "July 17, 2015", "October 3, 2012"];

/* Track description */
const midiTrackDescription = [
...
let midiAudio = new Audio();
let midiCurrentPlaylistIndex = 0;
let midiSeeking = false; // Determine if user is pressing the player midiSeeking bar
...
midiAudio.addEventListener('canplaythrough', function() {
	...
	$('#midi-end-time').text(secondToString(midiAudio.duration));
...
function seekTimeUpdateMidi(){
	const midiTime = (midiAudio.currentTime / midiAudio.duration) * 100;
	...
	if(midiSeeking === false){
		...
		if(isNaN(midiTime)){
			...
			$('#midi-current-time').text(secondToString(midiAudio.currentTime));
		}
	}
}
...
function refreshMidiTrackList(){
	const midiPlaylistChildren = $('#midi-tracklist').children().toArray();

	for(let i = 0; i <= midiPlaylist.length - 1; i++){
